...
RLMM: this packages lets you fit linear mixed effects models in a robust way. Implements the Robust Scoring Equations estimator for linear mixed effect models. - we used this package because we had issues with our model diagnostics (and transforming/trying other probability distributions did not resolve this)
...
# all basically give the same p-value (robust)
```
...
# calculate p-values based on robust t-values and non-robust approx. DFs
p.values <- 2*pt(abs(coefs.robust[,3]), coefs$df, lower=FALSE)
p.values # (slightly diff by 0.001)
```
